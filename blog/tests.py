...
from bs4 import BeautifulSoup # html을 분석할 수 있게 parsing 해주는 패키지
from django.contrib.auth.models import User # admin 갔을때 user page
...
class TestView(TestCase):
    ...
    def navbar_test(self, soup):
        navbar = soup.nav
        self.assertIn('Blog', navbar.text) # assertIn은 그 안에 있는지
        self.assertIn('About Me', navbar.text)

        logo_btn = navbar.find('a', text = "Do It Django") # 붙혀썼는데 띄어쓰기했으면 test 통과 X
    ...
    def test_post_list(self):
        ...
        response = self.client.get('/blog/') # 블로그 링크 가져오기
        # 1.2 정상적으로 페이지가 로드된다.
        self.assertEqual(response.status_code, 200) # 페이지 코드=200이 됌
        # 1.3 페이지 타이들은 'Blog'이다.
        soup = BeautifulSoup(response.content, 'html.parser') #response.content가 soup가 됌
        self.assertEqual(soup.title.text, 'Blog') # soup.title이 블로그가 맞는지 확인
        # 1.4 내이게이션 바가 있다.
                # navbar = soup.nav
